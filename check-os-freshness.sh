...
notify() {
    ...
    echo "${title}: ${body}"
    systemd-cat -t os-freshness -p "${urgency}" <<<"${title}: ${body}"
    command -v notify-send >/dev/null && \
        notify-send -u "$([[ ${urgency} == crit ]] && echo critical || echo normal)" \
            "${title}" "${body}" 2>/dev/null
}

read -r origin version < <(rpm-ostree status --json | python3 -c '
import json,sys
...
if [[ "${origin}" == *ghcr.io* ]]; then
    ref="${origin##*docker://}"
    remote_age="$(skopeo inspect "docker://${ref}" 2>/dev/null | python3 -c '
